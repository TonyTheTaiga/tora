...
{:else if role === "ADMIN"}
  <span
    class="text-xs px-2 py-0.5 bg-ctp-red/20 text-ctp-red border border-ctp-red/30 rounded-full font-medium flex-shrink-0"
  >
    ADMIN
  </span>
{:else if role === "EDITOR"}
...
{:else if role === "VIEWER"}
  <span
    class="text-xs px-2 py-0.5 bg-ctp-green/20 text-ctp-green border border-ctp-green/30 rounded-full font-medium flex-shrink-0"
  >
    VIEWER
  </span>
{/if}
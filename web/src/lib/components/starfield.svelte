    ...
    function createStars() {
      ...
      const material = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main() { float d = length(gl_PointCoord - vec2(0.5)); if (d > 0.5) discard; float twinkle = 0.7 + sin(time * 0.1 + vRandom * 20.0) * 0.3; float alpha = (1.0 - smoothstep(0.4, 0.5, d)) * twinkle; gl_FragColor = vec4(vColor, alpha); }`,
    ...
    layerData.forEach((data) => {
      ...
      const material = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          // OPTIMIZATION 3: Simplify the noise function

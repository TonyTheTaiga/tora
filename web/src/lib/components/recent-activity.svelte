...
<div class="space-y-1 font-mono text-xs">
  <div class="flex gap-4 text-ctp-subtext0 mb-2">
    <button
      ...
      class="transition-colors {activeTab === 'experiments' ? 'text-ctp-blue' : 'hover:text-ctp-text'}"
    >
      [experiments]
    </button>
    <button
      onclick={() => activeTab = 'workspaces'}
      class="transition-colors {activeTab === 'workspaces' ? 'text-ctp-blue' : 'hover:text-ctp-text'}"
    >
      [workspaces]
    </button>
  </div>
  ...
  <div class="space-y-0">
    {#if activeTab === 'experiments'}
      ...
      {#if experiments.length === 0}
        <div class="px-1 py-2 text-ctp-subtext0">
          no recent experiments
        </div>
      {/if}
    {:else}
      {#each workspaces.slice(0, 5) as workspace}
        <a href="/workspaces/{workspace.id}" class="block hover:bg-ctp-surface0/20 px-1 py-1 transition-colors">
          <div class="flex items-center gap-2">

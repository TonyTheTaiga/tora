<script lang="ts">
  ...
  import { resetWorkspaceToInvite } from "$lib/state/modal.svelte.js";
  ...
  function closeModal() {
    resetWorkspaceToInvite();
  }
</script>

<BaseModal title="Invite User" onClose={closeModal}>
  <form
    method="POST"
    ...
    <ModalFormSection title="details">
      <div class="flex items-center gap-2 mb-3">
        <span class="text-sm text-ctp-subtext0"
          >workspace: <span class="text-ctp-blue">{workspace.name}</span></span
        >
      </div>

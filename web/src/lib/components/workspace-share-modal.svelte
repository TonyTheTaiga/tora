  ...
  const roles: { value: WorkspaceRole; label: string; description: string }[] =
    [
      { value: "VIEWER", label: "Viewer", description: "Can view experiments" },
      {
        value: "EDITOR",
        label: "Editor",
        description: "Can create and edit experiments",
      },
      {
        value: "ADMIN",
        label: "Admin",
      ...
      <form onsubmit={handleInvite} class="space-y-4">
        ...
        <div>
          ...
          <div class="relative">
            ...
            >
              ...
              <ChevronDown
                size={16}
                class="text-ctp-subtext0 transition-transform duration-200 {dropdownOpen
                  ? 'rotate-180'
                  : ''}"
              />
            </button>
            ...
            {#if dropdownOpen}
              <div
                class="absolute top-full left-0 right-0 mt-1 bg-ctp-surface0/80 backdrop-blur-xl border border-ctp-surface1/20 rounded-xl shadow-xl z-10"
              >
                {#each roles as role}
                  <button
                    type="button"
        ...
        <div class="flex gap-3 pt-4">
          ...
          >
            {#if isLoading}
              <div
                class="w-4 h-4 border-2 border-ctp-blue/30 border-t-ctp-blue rounded-full animate-spin"
              ></div>
            {:else}
              <Send size={16} />
            {/if}

...
<div
  class="bg-ctp-surface0/10 backdrop-blur-md rounded-2xl border border-ctp-surface0/20 p-6 shadow-xl"
>
  ...
  {:else}
    <div class="space-y-3">
      {#each members as member}
        <div
          class="flex items-center justify-between p-4 bg-ctp-surface0/20 backdrop-blur-sm rounded-xl border border-ctp-surface0/30 hover:border-ctp-surface0/50 transition-all"
        >
          <div class="flex items-center gap-3">
            <div
              class="w-10 h-10 bg-ctp-surface1/50 rounded-full flex items-center justify-center"
            >
            ...
            <div>
              ...
              <div class="flex items-center gap-2">
                <span
                  class="text-xs px-2 py-1 rounded-full border {roleColors[
                    member.role
                  ]}">{member.role}</span
                >
              </div>
          ...
          {#if workspace.role === "OWNER" && member.role !== "OWNER"}
            <div class="relative member-dropdown">
              ...
              {#if dropdownOpen === member.id}
                <div
                  class="absolute right-0 top-full mt-1 bg-ctp-surface0/80 backdrop-blur-xl border border-ctp-surface1/20 rounded-xl shadow-xl z-10 min-w-40"
                >
                  <div class="p-2">
                    <div
                      class="text-xs text-ctp-subtext0 uppercase tracking-wider px-2 py-1 mb-2"
                    >
                      Change Role
                    </div>

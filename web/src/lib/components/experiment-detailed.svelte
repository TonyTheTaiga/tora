        ...
        >
          ...
          <div class="flex gap-1.5 flex-wrap"> {/* Ensured wrap and gap */}
            {#each experiment.tags as tag}
              <span
                class="whitespace-nowrap inline-flex items-center px-2 py-1 text-xs bg-ctp-surface0 text-ctp-blue rounded-full" /* Adjusted padding and colors */
              >
                {tag}
              </span>
            {/each}
                      ...
                      >
                        ...
                        <td
                          ...
                          >{typeof metric.value === "number"
                            ...
                            : metric.value}</td
                        >
                        <td class="p-3 text-ctp-subtext1">{metric.step ?? "N/A"}</td> {/* Consistent display for null step */}
